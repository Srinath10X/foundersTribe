...
export async function promoteUser(actorId, targetId, roomId, newRole) {
    ...
    if (ROLE_HIERARCHY[newRole] >= ROLE_HIERARCHY[actor.role]) {
        throw new AppError('Cannot promote to equal or higher role than your own', 403);
    }
    ...
    const updated = await participantRepository.updateParticipant(roomId, targetId, {
        role: newRole,
...
export async function revokeMic(actorId, targetId, roomId) {
    ...
    if (ROLE_HIERARCHY[target.role] >= ROLE_HIERARCHY[actor.role]) {
        throw new AppError('Cannot revoke mic from a user with equal or higher role', 403);
    }
