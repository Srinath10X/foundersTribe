...
async function fetchGigUsers(token) {
    try {
        ...
        return (json.items || []).filter((u) => u.role === "freelancer" || u.role === "both");
    }
    catch {
...
async function fetchFreelancerServices(freelancerId, token) {
    try {
        const res = await fetch(`${env.GIG_SERVICE_URL}/api/services/freelancers/${freelancerId}`, {
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
            },
 ...
 */
export async function fetchFreelancerPool(token) {
    // Fetch from profiles table (tribe-service) — freelancers & both
    const { data: profiles } = await supabaseAdmin
        .from("profiles")
        .select("id, display_name, username, photo_url, avatar_url, bio, user_type, role, location, rating, previous_works, completed_gigs, linkedin_url, business_idea, business_ideas, social_links, contact")
    ...
    const serviceMap = {};
    await Promise.all(Array.from(freelancerIds)
        .slice(0, 50)
        .map(async (fid) => {
 ...
 */
export function buildFreelancerContext(profiles, userProfiles, services, idMap) {
    const seen = new Set();
    const entries = [];
    let counter = 1;
    const getShortId = (realId) => {
    ...
    for (const p of profiles) {
        ...
        const prevWorks = Array.isArray(p.previous_works) && p.previous_works.length > 0
            ? p.previous_works
                .map((w) => {
        ...
        const completedGigs = Array.isArray(p.completed_gigs) && p.completed_gigs.length > 0
            ? p.completed_gigs
                .map((g) => {
        ...
        const businessIdeas = Array.isArray(p.business_ideas) && p.business_ideas.length > 0
            ? p.business_ideas
                .map((idea) => {
                const title = typeof idea === "string"
                    ? idea
            ...
            : "";
        const svcList = svcs
            .map((s) => `  - ${s.service_name}: ${s.cost_currency}${s.cost_amount} / ${s.delivery_time_value} ${s.delivery_time_unit}${s.description ? ` — ${s.description}` : ""}`)
            .join("\n");
        entries.push([
            `ID: ${shortId}`,
            `Name: ${name}`,
