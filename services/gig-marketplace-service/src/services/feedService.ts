...
export async function createPost(
  ...
  try {
    ...
    if (!existing) {
      ...
      if (!existing.full_name && userMeta?.full_name) updates.full_name = userMeta.full_name;
      if (!existing.avatar_url && userMeta?.avatar_url) updates.avatar_url = userMeta.avatar_url;
...
export async function listFeed(db: SupabaseClient, query: Record<string, any>, userId: string) {
  const repo = new FeedRepository(db);
  ...
  if (items.length > 0) {
    ...
    const [{ data: profiles }, { data: likes }] = await Promise.all([
      ...
      db
        ...
        .select("post_id")
        .eq("user_id", userId)
        .in("post_id", items.map((p: any) => p.id)),
    ]);
...
export async function getPostById(db: SupabaseClient, id: string, userId: string) {
  const repo = new FeedRepository(db);
  ...
  if (tribeProfile) {
    (post as any).author = {
      ...
      full_name: tribeProfile.display_name || (post as any).author?.full_name || null,
      avatar_url: tribeProfile.photo_url || tribeProfile.avatar_url || (post as any).author?.avatar_url || null,
      handle: tribeProfile.username || (post as any).author?.handle || null,
    };
  }
...
export async function deletePost(db: SupabaseClient, id: string, authorId: string) {
  try {
    const repo = new FeedRepository(db);
    const post = await repo.getPostById(id);
    if (!post) throw new AppError("Post not found", 404, "not_found");
    if (post.author_id !== authorId) throw new AppError("Unauthorized", 403, "forbidden");
...
export async function likePost(db: SupabaseClient, postId: string, userId: string) {
  try {
    const repo = new FeedRepository(db);
    ...
    if (existing) throw new AppError("Already liked", 409, "conflict");
...
export async function unlikePost(db: SupabaseClient, postId: string, userId: string) {
  try {
    const repo = new FeedRepository(db);
...
export async function listComments(db: SupabaseClient, postId: string, query: Record<string, any>) {
  const repo = new FeedRepository(db);
  const limit: number = Math.min(Number(query.limit || 20), 100);
  const cursor = decodeCursor(query.cursor);
