...
export class ContractRepository {
  ...
  private async hydrateContractRows(rows: ContractRow[]): Promise<any[]> {
    ...
    const [gigsRes, proposalsRes, profilesRes] = await Promise.all([
      gigIds.length
        ? supabaseAdmin
            .from("gigs")
            .select(
              "id, title, description, budget_min, budget_max, status, is_remote, location_text, experience_level, founder:user_profiles!gigs_founder_id_fkey(id, full_name, avatar_url, handle), gig_tags(tag_id, tags(id, slug, label))",
            )
            .in("id", gigIds)
    ...
    const gigMap = new Map((gigsRes.data || []).map((item: any) => [item.id, item]));
    const proposalMap = new Map((proposalsRes.data || []).map((item: any) => [item.id, item]));
  ...
  async listContracts(filters: Record<string, any>, limit: number, cursor: { createdAt: string, id: string } | null) {
    ...
    return this.hydrateContractRows((data || []) as ContractRow[]);
  }
  ...
  async getContractById(id: string) {
    ...
    if (error) throw error;
    if (!data) return data;
    const [enriched] = await this.hydrateContractRows([data as ContractRow]);
    return enriched;
  }
