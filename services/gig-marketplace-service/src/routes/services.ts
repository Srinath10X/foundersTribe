...
import {
  ...
  getFreelancerServicesByUserSchema,
  listFreelancerServicesSchema,
...
router.get("/freelancers/:id", validate(getFreelancerServicesByUserSchema), async (req, res, next) => {
  try {
    const data = await serviceCatalogService.listFreelancerServicesByUserId(req.db, req.params.id as string);
    res.json({ items: data });
  } catch (err) {
    next(err);
  }
...
router.get("/requests/:id", validate(getServiceRequestSchema), async (req, res, next) => {
  try {
    const data = await serviceCatalogService.getServiceRequestById(req.db, req.params.id as string, req.user.id);
    res.json({ data });
  } catch (err) {
    next(err);
  }
...
router.get("/requests/:id/messages", validate(listServiceRequestMessagesSchema), async (req, res, next) => {
  try {
    const data = await serviceCatalogService.listServiceRequestMessages(
      req.db,
...
router.post("/requests/:id/messages", validate(createServiceRequestMessageSchema), async (req, res, next) => {
  try {
    const data = await serviceCatalogService.createServiceRequestMessage(
      req.db,
      req.params.id as string,
      req.user.id,
      req.body,
