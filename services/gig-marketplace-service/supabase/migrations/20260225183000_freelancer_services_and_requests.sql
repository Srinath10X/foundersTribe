...
create or replace function public.ensure_user_profile_stub(p_user_id uuid)
returns void
language plpgsql
security definer
...
begin
  perform public.ensure_user_profile_stub(p_user_id);
...
revoke all on function public.ensure_user_profile_stub(uuid) from public;
grant execute on function public.ensure_user_profile_stub(uuid) to service_role;
