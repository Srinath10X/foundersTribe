...
import { createPostSchema, listFeedSchema, getPostSchema, deletePostSchema, likePostSchema, createCommentSchema, listCommentsSchema, } from "../schemas/feedSchemas.js";
...
router.get("/:id", validate(getPostSchema), async (req, res, next) => {
    try {
        const data = await feedService.getPostById(req.db, req.params.id, req.user.id);
        res.json({ data });
    }
    catch (err) {
        next(err);
...
router.delete("/:id", validate(deletePostSchema), async (req, res, next) => {
    try {
        await feedService.deletePost(req.db, req.params.id, req.user.id);
        res.status(204).send();
    }
    catch (err) {
        next(err);
...
router.post("/:id/like", validate(likePostSchema), async (req, res, next) => {
    try {
        const data = await feedService.likePost(req.db, req.params.id, req.user.id);
        res.json({ data });
    }
    catch (err) {
        next(err);
...
router.delete("/:id/like", validate(likePostSchema), async (req, res, next) => {
    try {
        const data = await feedService.unlikePost(req.db, req.params.id, req.user.id);
        res.json({ data });
    }
    catch (err) {
        next(err);
...
router.post("/:id/comments", validate(createCommentSchema), async (req, res, next) => {
    try {
        const data = await feedService.createComment(req.db, req.params.id, req.user.id, req.body);
        res.status(201).json({ data });
    }
    catch (err) {
        next(err);
...
router.get("/:id/comments", validate(listCommentsSchema), async (req, res, next) => {
    try {
        const data = await feedService.listComments(req.db, req.params.id, req.query);
        res.json(data);
    }
    catch (err) {
        next(err);
