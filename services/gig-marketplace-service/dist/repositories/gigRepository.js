export class GigRepository {
    ...
    async getFreelancerStats(userId) {
        ...
        if (completedErr)
            throw completedErr;
        const earningsMtd = (completedContracts || []).reduce((sum, c) => sum + (Number(c.proposals?.proposed_amount) || 0), 0);
        return {
    ...
    async addTagsToGig(gigId, tagLabels) {
    ...
    async listGigs(filters, limit, cursorParts) {
        ...
        if (filters.startup_stage)
            query = query.eq("startup_stage", filters.startup_stage);
        if (filters.budget_min)
            query = query.gte("budget_min", Number(filters.budget_min));
        if (filters.budget_max)
            query = query.lte("budget_max", Number(filters.budget_max));
        if (filters.gig_ids)
            query = query.in("id", filters.gig_ids);
        if (filters.founder_id)
            query = query.eq("founder_id", filters.founder_id);
        if (cursorParts) {
