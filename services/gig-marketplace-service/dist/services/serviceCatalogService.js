...
function durationInDays(service) {
...
export async function listMyFreelancerServices(db, userId) {
    const repo = new ServiceRepository(db);
    return repo.listMyServices(userId);
}
export async function listFreelancerServicesByUserId(db, freelancerId) {
    const repo = new ServiceRepository(db);
    return repo.listFreelancerServices(freelancerId);
}
export async function upsertMyFreelancerServices(db, userId, payload) {
    try {
        const repo = new ServiceRepository(db);
        const services = (payload.services || []).map((row) => ({
            ...
            delivery_time_unit: (row.delivery_time_unit === "weeks" ? "weeks" : "days"),
...
export async function searchFreelancerServices(db, query) {
    const repo = new ServiceRepository(db);
    ...
    const maxDeliveryDays = query.max_delivery_days !== undefined ? Math.max(1, Math.trunc(toNumber(query.max_delivery_days, 1))) : undefined;
    ...
    const specificFreelancerId = typeof query.freelancer_id === "string" && query.freelancer_id.trim()
    ...
    if (cursor) {
        items = items.filter((item) => {
            ...
            if (Number.isNaN(itemTs) || Number.isNaN(cursorTs))
                return true;
            if (itemTs < cursorTs)
                return true;
            if (itemTs > cursorTs)
...
export async function createServiceRequest(db, founderId, payload) {
    try {
        const repo = new ServiceRepository(db);
        ...
        const buildInitialMessage = (serviceName) => {
            const explicit = (payload.message || "").trim();
            if (explicit)
                return explicit;
            if (serviceName)
                return `Hi, I would like to discuss your ${serviceName} service.`;
...
export async function listServiceRequests(db, userId, query) {
    const repo = new ServiceRepository(db);
...
export async function getServiceRequestById(db, requestId, userId) {
    const repo = new ServiceRepository(db);
...
export async function listServiceRequestMessages(db, requestId, userId, query) {
    const repo = new ServiceRepository(db);
...
export async function createServiceRequestMessage(db, requestId, userId, payload) {
    try {
        const repo = new ServiceRepository(db);
        const request = await repo.getServiceRequestById(requestId);
        if (!request)
...
async function transitionServiceRequestStatus(db, requestId, userId, targetStatus) {
    const repo = new ServiceRepository(db);
    const request = await repo.getServiceRequestById(requestId);
    if (!request)
        throw new AppError("Request not found", 404, "not_found");
...
export async function acceptServiceRequest(db, requestId, userId) {
    try {
...
export async function declineServiceRequest(db, requestId, userId) {
    try {
...
export async function markServiceRequestMessagesRead(db, requestId, userId) {
    try {
        const repo = new ServiceRepository(db);
        const request = await repo.getServiceRequestById(requestId);
        if (!request)
