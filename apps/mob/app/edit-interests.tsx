...
export default function EditInterests() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const response = await fetch(
        "http://192.168.0.19:3000/api/get_all_categories",
        {
          headers: {
      ...
      const fetchedCats = data.map((item: any) => ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // convert selected IDs → category labels
      const selectedCategories = selected.map((catId) => {
        const cat = categories.find((c) => c.id === catId);
        return cat ? cat.label : catId;
      });

      const response = await fetch(
        "http://192.168.0.19:3000/api/user_categories",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${session.access_token}`,
